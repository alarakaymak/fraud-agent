...
The LangGraph supervisor uses Claude 3.5 Haiku via Amazon Bedrock. It is responsible for reading all active specialist reports and producing the final structured decision. The decision logic uses a hybrid approach: clear-cut BLOCK conditions are enforced deterministically in Python before the LLM is invoked, while nuanced decisions (APPROVE, REVIEW, CLARIFICATION_NEEDED) are left to the supervisor's judgment. Specifically, if two or more specialists report HIGH risk, or if the ML score exceeds 0.80 with at least one HIGH signal, the system issues a BLOCK immediately without an LLM call. This prevents the model from rationalizing its way out of clear fraud signals. For all remaining cases, the supervisor LLM weighs the specialist findings against the fraud score and decides.
...
Second, the supervisor synthesizes the selected specialist reports, which often conflict — a high velocity signal might come alongside a normal location signal, or an unusual amount might appear at a perfectly normal time of day. For the clearest fraud cases (two or more HIGH signals), the decision is enforced deterministically before the LLM is invoked. For everything else — approvals, borderline cases, and verification requests — the supervisor LLM weighs the signals against each other and the fraud score to decide. That judgment requires genuine reasoning: a single HIGH spending signal at home in a familiar city with a moderate score is a different situation from the same signal in an unfamiliar city at 3 AM, and the supervisor handles them differently.
...
Claude 3.5 Haiku was used for all LLM components — triage, specialist agents, supervisor, and the cardholder conversation endpoint. Haiku was chosen over Sonnet for cost efficiency and latency: the structured nature of the specialist reports and the well-defined decision criteria mean the task does not require Sonnet-level reasoning. For the nuanced decisions — distinguishing REVIEW from CLARIFICATION_NEEDED, or recognizing that a high-value purchase fits a known customer's pattern — Haiku performs reliably with explicit structured inputs. For the hard rule cases (two HIGH signals = BLOCK), enforcement was moved to Python after testing showed the model could rationalize its way to softer decisions even with clear instructions.
...
One operational note specific to the demo: the card testing scenario uses a two-layer velocity check. The primary check counts transactions in a rolling 60-minute window; the secondary check scans the most recent transactions for burst density regardless of absolute time (3 or more transactions within any 15-minute span). This means the burst pattern is detectable from the relative density of seeded records even after the 60-minute rolling window has expired. In a production deployment this would not be a concern — real transaction history populates continuously and both checks always reflect genuine recent activity.
...
I expected the LLM to be unpredictable — that it would give different answers every time I ran the same input. The consistency tests showed that was only true at the decision boundary. For anything with a clear signal in either direction, the system was extremely stable. A known high-spender at their usual merchant returned APPROVE every time. The LLM was only inconsistent on the cases where even a human analyst would hesitate — scores in the 0.35–0.55 range where the signals genuinely conflict.
...
**When to enforce rules in code rather than prompts**

One of the more practical lessons from this project was learning which decisions to leave to the LLM and which to enforce deterministically in code. My initial assumption was that a well-written prompt could handle everything — if I described the decision rules clearly enough, the model would follow them. That assumption turned out to be wrong for one specific case: when two or more specialists returned HIGH risk, the supervisor would sometimes reason its way to a softer outcome. It might note that the transaction was in the cardholder's home city, or that the ML score was not extreme, and return REVIEW instead of BLOCK. Technically this reasoning was not wrong — those are mitigating factors. But the system was designed to treat two HIGH signals as a hard stop regardless of context.

The fix was to move BLOCK enforcement out of the prompt and into Python. Before the LLM is invoked at all, the code counts the HIGH signals and issues BLOCK immediately if the threshold is met. The LLM never sees those cases. Similarly, the temporal specialist's risk classification — whether 11 PM is MEDIUM risk and 2 AM is HIGH — was made deterministic in the tool rather than leaving it to the model's interpretation. The result was that the LLM's job became scoped to genuinely ambiguous decisions, where its reasoning ability actually adds value, rather than also handling clear-cut cases where consistency matters more than judgment. A useful rule of thumb: if you can write the decision as an if-statement and it covers all the cases you care about, write the if-statement. Reserve the LLM for cases where the right answer depends on context you cannot enumerate in advance.
